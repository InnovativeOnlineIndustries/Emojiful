...
public class EmojiFontRenderer extends Font {
    ...
    @Override
    private float renderText(String text, float x, float y, int color, boolean isShadow, Matrix4f matrix, MultiBufferSource buffer, boolean isTransparent, int colorBackgroundIn, int packedLight) {
        if (text.isEmpty())
            return 0;
        HashMap<Integer, Emoji> emojis = new LinkedHashMap<>();
        try {
    ...
    class EmojiCharacterRenderer implements FormattedCharSink {
        ...
        public boolean accept(int pos, Style style, int charInt) {
            FontAccessor fontAccessor = ((FontAccessor)(EmojiFontRenderer.this));
            FontSet font =  fontAccessor.emojifulCommon_getFontSet(style.getFont());
            if (EmojifulConfig.getInstance().renderEmoji.get() && this.emojis.get(pos) != null) {
                ...
                GlyphInfo iglyph = font.getGlyphInfo(charInt, fontAccessor.emojifulCommon_getFishyGlyphs());
